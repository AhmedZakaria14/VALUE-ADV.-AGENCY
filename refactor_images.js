...
// Replace exactly:
// const allImages = Array.from({ length: 169 }, (_, i) => `/portfolio/work-${i + 1}.jpeg`);
...
// In PortfolioGallery.tsx mapping
// {allImages.slice(0, visibleCount).map((img, index) => ( ... key={img} ... src={img} ... alt={`عمل ${index + 1}`} ...
// Replace img map parameter with item, and img string with item.src
...
// Lightbox image replacement
// src={allImages[selectedImageIndex]} -> src={allImages[selectedImageIndex].src}
// alt={`عمل ${selectedImageIndex + 1}`} -> alt={allImages[selectedImageIndex].alt}
...
// Wait, the previous string replacement of img -> item.src globally might break things like <Image ... /> or other mappings
// Let me be more precise for HomeClient.tsx mapping replacement
`;

// Undo the img replacement globally
// Let's rewrite the script properly
